import Link from "next/link";
import CrystalCard from "@/components/CrystalCard";
import BlurText from "@/components/BlurText";
import GoldButton from "@/components/GoldButton";
import ScrollReveal from "@/components/ScrollReveal";
import AuroraBackground from "@/components/AuroraBackground";

export default function ContactPage() {
  return (
    <div className="flex-1">
      {/* Hero Section */}
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden bg-black">
        <div className="absolute inset-0 bg-gradient-to-b from-surface-card to-black" />
        <div
          className="absolute inset-0 opacity-20"
          style={{
            background: `
              radial-gradient(ellipse 60% 50% at 50% 0%, rgba(185, 28, 62, 0.08) 0%, transparent 60%)
            `,
          }}
        />

        <div className="section-container relative z-10 text-center py-20">
          <div className="flex items-center justify-center gap-3 mb-6">
            <div className="w-8 h-[1px] bg-accent" />
            <span className="text-xs tracking-[0.3em] uppercase text-gray-500 font-inter">Contact</span>
            <div className="w-8 h-[1px] bg-accent" />
          </div>
          <h1 className="text-5xl md:text-7xl font-playfair font-bold mb-6 text-white">
            <BlurText text="Get In Touch" speed={50} delay={300} />
          </h1>
          <p className="text-xl text-gray-400 max-w-3xl mx-auto font-inter">
            Book your session, request a fitting, or get answers to your questions
          </p>
        </div>
      </section>

      {/* Contact Info — 3 CrystalCard tiles */}
      <section className="section-padding bg-black">
        <div className="section-container">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            <ScrollReveal>
              <CrystalCard className="p-8 text-center" hover="glow">
                <div className="w-20 h-20 mx-auto bg-accent/10 rounded-2xl flex items-center justify-center text-4xl mb-6">
                  📅
                </div>
                <h3 className="text-2xl font-playfair font-bold text-white mb-4">Book A Session</h3>
                <p className="text-gray-500 mb-6 font-inter">
                  In-person coaching in Leeds or online coaching from anywhere
                </p>
                <a href="https://calendly.com/james-whittaker-golf" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-accent hover:text-accent-light transition-colors font-playfair">
                  📅 Book Now
                </a>
              </CrystalCard>
            </ScrollReveal>

            <ScrollReveal delay={100}>
              <CrystalCard className="p-8 text-center" hover="glow">
                <div className="w-20 h-20 mx-auto bg-accent/10 rounded-2xl flex items-center justify-center text-4xl mb-6">
                  📧
                </div>
                <h3 className="text-2xl font-playfair font-bold text-white mb-4">Free Assessment</h3>
                <p className="text-gray-500 mb-6 font-inter">
                  Upload your swing and get ONE clear fix within 48 hours
                </p>
                <a href="mailto:james@jwgolf.co.uk" className="inline-flex items-center gap-2 text-accent hover:text-accent-light transition-colors font-playfair">
                  📧 james@jwgolf.co.uk
                </a>
              </CrystalCard>
            </ScrollReveal>

            <ScrollReveal delay={200}>
              <CrystalCard className="p-8 text-center" hover="glow">
                <div className="w-20 h-20 mx-auto bg-accent/10 rounded-2xl flex items-center justify-center text-4xl mb-6">
                  📍
                </div>
                <h3 className="text-2xl font-playfair font-bold text-white mb-4">Leeds Facility</h3>
                <p className="text-gray-500 mb-6 font-inter">
                  Private coaching facility in Leeds, Yorkshire
                </p>
                <p className="text-accent font-playfair">Leeds, Yorkshire</p>
              </CrystalCard>
            </ScrollReveal>
          </div>
        </div>
      </section>

      {/* Booking Form */}
      <section id="booking" className="section-padding bg-surface">
        <div className="section-container max-w-4xl mx-auto">
          <div className="section-header mb-12">
            <h2 className="section-title font-playfair">Book Your Session</h2>
            <p className="section-subtitle font-inter">
              Complete the form below and I&apos;ll get back to you within 24 hours to
              confirm your session time
            </p>
          </div>

          <ScrollReveal>
            <CrystalCard variant="gold" className="p-8" hover="glow">
              <form>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="form-label">First Name</label>
                    <input
                      type="text"
                      name="firstName"
                      className="form-input"
                      placeholder="James"
                    />
                  </div>
                  <div>
                    <label className="form-label">Last Name</label>
                    <input
                      type="text"
                      name="lastName"
                      className="form-input"
                      placeholder="Whittaker"
                    />
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label">Email Address</label>
                  <input
                    type="email"
                    name="email"
                    className="form-input"
                    placeholder="james@example.com"
                  />
                </div>

                <div className="mb-6">
                  <label className="form-label">Phone Number</label>
                  <input
                    type="tel"
                    name="phone"
                    className="form-input"
                    placeholder="07810 647976"
                  />
                </div>

                <div className="mb-6">
                  <label className="form-label">Booking Type</label>
                  <select className="form-input">
                    <option value="">Select a session type</option>
                    <option value="in-person">In-Person Coaching (Leeds)</option>
                    <option value="online">Online Coaching</option>
                    <option value="fitting">Custom Fitting</option>
                    <option value="playing">Playing Lesson</option>
                    <option value="consultation">Free Consultation</option>
                  </select>
                </div>

                <div className="mb-6">
                  <label className="form-label">Handicap</label>
                  <select className="form-input">
                    <option value="">Select your handicap</option>
                    <option value="0-5">0-5 (Low Handicap)</option>
                    <option value="6-10">6-10 (Mid Handicap)</option>
                    <option value="11-18">11-18 (High Handicap)</option>
                    <option value="19+">19+ (Beginner)</option>
                    <option value="lady-low">Lady Low (0-5)</option>
                    <option value="lady-mid">Lady Mid (6-10)</option>
                    <option value="lady-high">Lady High (11-18)</option>
                    <option value="lady-beginner">Lady Beginner (19+)</option>
                    <option value="junior">Junior (Under 18)</option>
                    <option value="senior">Senior (65+)</option>
                    <option value="not-sure">Not sure / New to golf</option>
                  </select>
                </div>

                <div className="mb-6">
                  <label className="form-label">What brings you to golf?</label>
                  <select className="form-input">
                    <option value="">Select an option</option>
                    <option value="beginner">I&apos;m a complete beginner</option>
                    <option value="improve">I want to improve my game</option>
                    <option value="fix-slice">I want to fix my slice</option>
                    <option value="lower-handicap">I want to lower my handicap</option>
                    <option value="fitting">I need equipment fitting</option>
                    <option value="mental-game">I want help with the mental game</option>
                    <option value="returning">I&apos;m returning to golf after break</option>
                    <option value="other">Other (please let me know in message)</option>
                  </select>
                </div>

                <div className="mb-8">
                  <label className="form-label">Message</label>
                  <textarea
                    name="message"
                    className="form-textarea"
                    placeholder="Tell me about your golf goals..."
                  />
                </div>

                <CrystalCard variant="green" className="flex items-start gap-3 mb-8 p-4">
                  <div className="w-5 h-5 rounded bg-accent/20 flex items-center justify-center text-accent flex-shrink-0">
                    ✓
                  </div>
                  <div className="text-sm text-gray-500 font-inter">
                    <p className="mb-2"><strong className="text-gray-300">Within 24 hours:</strong> I&apos;ll contact you to confirm your session time</p>
                    <p><strong className="text-gray-300">Free swing assessment:</strong> With any coaching package booking</p>
                  </div>
                </CrystalCard>

                <GoldButton type="submit" variant="primary" size="lg" className="w-full">
                  Request Booking
                </GoldButton>

                <p className="text-center text-sm text-gray-600 mt-4 font-inter">
                  Or <Link href="https://calendly.com/james-whittaker-golf" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">
                    book directly via Calendly
                  </Link>
                </p>
              </form>
            </CrystalCard>
          </ScrollReveal>
        </div>
      </section>

      {/* Free Video Assessment Form */}
      <section id="online" className="section-padding bg-black">
        <div className="section-container max-w-4xl mx-auto">
          <div className="section-header mb-12 text-center">
            <h2 className="section-title text-white font-playfair">Free Video Assessment</h2>
            <p className="section-subtitle font-inter">
              Upload your swing video. Get ONE clear fix to work on. 48-hour turnaround.
            </p>
          </div>

          <ScrollReveal>
            <CrystalCard className="p-8" hover="glow">
              <form>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="form-label">First Name</label>
                    <input
                      type="text"
                      name="firstName"
                      className="form-input"
                      placeholder="James"
                    />
                  </div>
                  <div>
                    <label className="form-label">Last Name</label>
                    <input
                      type="text"
                      name="lastName"
                      className="form-input"
                      placeholder="Whittaker"
                    />
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label">Email Address</label>
                  <input
                    type="email"
                    name="email"
                    className="form-input"
                    placeholder="james@example.com"
                  />
                </div>

                <div className="mb-6">
                  <label className="form-label">Handicap</label>
                  <select className="form-input">
                    <option value="">Select your handicap</option>
                    <option value="0-5">0-5</option>
                    <option value="6-10">6-10</option>
                    <option value="11-18">11-18</option>
                    <option value="19+">19+</option>
                    <option value="lady-low">Lady Low (0-5)</option>
                    <option value="lady-mid">Lady Mid (6-10)</option>
                    <option value="lady-high">Lady High (11-18)</option>
                    <option value="lady-beginner">Lady Beginner (19+)</option>
                    <option value="junior">Junior</option>
                    <option value="senior">Senior</option>
                    <option value="not-sure">Not sure</option>
                  </select>
                </div>

                <div className="mb-6">
                  <label className="form-label">Upload Swing Video</label>
                  <div className="border-2 border-dashed border-white/[0.10] rounded-lg p-8 text-center hover:border-accent/40 transition-colors cursor-pointer">
                    <div className="text-5xl mb-4">🎥</div>
                    <p className="text-white font-medium font-playfair">Click or drag video to upload</p>
                    <p className="text-sm text-gray-600 mt-2 font-inter">MP4, MOV, or M4V (max 50MB)</p>
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label">What would you like help with?</label>
                  <select className="form-input">
                    <option value="">Select what you need help with</option>
                    <option value="full-swing">Full swing (driver, irons, woods)</option>
                    <option value="short-game">Short game (chipping, pitching, bunker)</option>
                    <option value="putting">Putting</option>
                    <option value="course-management">Course management</option>
                    <option value="mental-game">Mental game</option>
                    <option value="other">Other (please specify below)</option>
                  </select>
                </div>

                <div className="mb-8">
                  <label className="form-label">Additional Questions or Goals</label>
                  <textarea
                    name="message"
                    className="form-textarea"
                    placeholder="Tell me what you want to improve..."
                  />
                </div>

                <GoldButton type="submit" variant="primary" size="lg" className="w-full">
                  Submit For Assessment
                </GoldButton>

                <p className="text-center text-sm text-gray-500 mt-4 font-inter">
                  I&apos;ll analyze your swing and send you a personalized video within 48 hours.
                </p>
              </form>
            </CrystalCard>
          </ScrollReveal>
        </div>
      </section>

      {/* Fitting Booking */}
      <section id="fitting" className="section-padding bg-surface">
        <div className="section-container max-w-4xl mx-auto">
          <div className="section-header mb-12 text-center">
            <h2 className="section-title font-playfair">Custom Club Fitting</h2>
            <p className="section-subtitle font-inter">
              The same fitting the pros get. Trackman-certified equipment analysis.
            </p>
          </div>

          <ScrollReveal>
            <CrystalCard variant="gold" className="p-8" hover="glow">
              <form>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="form-label">First Name</label>
                    <input type="text" name="firstName" className="form-input" placeholder="James" />
                  </div>
                  <div>
                    <label className="form-label">Last Name</label>
                    <input type="text" name="lastName" className="form-input" placeholder="Whittaker" />
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label">Email Address</label>
                  <input type="email" name="email" className="form-input" placeholder="james@example.com" />
                </div>

                <div className="mb-6">
                  <label className="form-label">Phone Number</label>
                  <input type="tel" name="phone" className="form-input" placeholder="07810 647976" />
                </div>

                <div className="mb-6">
                  <label className="form-label">Fitting Type</label>
                  <select className="form-input">
                    <option value="">Select equipment to fit</option>
                    <option value="full-bag">Full Bag Fitting</option>
                    <option value="driver-fairway">Driver & Fairway Woods</option>
                    <option value="irons">Iron Fitting</option>
                    <option value="wedges-putter">Wedges & Putter</option>
                    <option value="driver-only">Driver Only</option>
                    <option value="other">Other</option>
                  </select>
                </div>

                <div className="mb-6">
                  <label className="form-label">Current Clubs</label>
                  <select className="form-input">
                    <option value="">Are your clubs currently fitted?</option>
                    <option value="fitted">Yes, completely fitted</option>
                    <option value="mostly-fitted">Mostly fitted</option>
                    <option value="some-fitted">Some fitted</option>
                    <option value="not-fitted">Not fitted at all</option>
                    <option value="new-golfer">I&apos;m new to golf</option>
                  </select>
                </div>

                <div className="mb-8">
                  <label className="form-label">Goals For Fitting</label>
                  <textarea
                    name="fittingGoals"
                    className="form-textarea"
                    placeholder="What do you want to improve with your equipment..."
                  />
                </div>

                <GoldButton type="submit" variant="primary" size="lg" className="w-full">
                  Request Fitting Session
                </GoldButton>

                <p className="text-center text-sm text-gray-600 mt-4 font-inter">
                  I&apos;ll contact you within 24 hours to confirm your fitting appointment.
                </p>
              </form>
            </CrystalCard>
          </ScrollReveal>
        </div>
      </section>

      {/* Playing Lesson Booking */}
      <section id="playing" className="section-padding bg-black">
        <div className="section-container max-w-4xl mx-auto">
          <div className="section-header mb-12 text-center">
            <h2 className="section-title text-white font-playfair">Playing Lessons</h2>
            <p className="section-subtitle font-inter">
              Learn on the course. Apply your swing changes to real-game situations.
            </p>
          </div>

          <ScrollReveal>
            <CrystalCard className="p-8" hover="glow">
              <form>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="form-label">First Name</label>
                    <input type="text" name="firstName" className="form-input" placeholder="James" />
                  </div>
                  <div>
                    <label className="form-label">Last Name</label>
                    <input type="text" name="lastName" className="form-input" placeholder="Whittaker" />
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label">Email Address</label>
                  <input type="email" name="email" className="form-input" placeholder="james@example.com" />
                </div>

                <div className="mb-6">
                  <label className="form-label">Phone Number</label>
                  <input type="tel" name="phone" className="form-input" placeholder="07810 647976" />
                </div>

                <div className="mb-6">
                  <label className="form-label">Preferred Course</label>
                  <select className="form-input">
                    <option value="">Select your preferred course</option>
                    <option value="leeds-gc">Leeds GC</option>
                    <option value="royal-leamington">Royal Leamington Spa</option>
                    <option value="hotwells">Hotwells Park</option>
                    <option value="other">Other (please specify)</option>
                  </select>
                </div>

                <div className="mb-6">
                  <label className="form-label">Handicap</label>
                  <select className="form-input">
                    <option value="">Select your handicap</option>
                    <option value="0-5">0-5</option>
                    <option value="6-10">6-10</option>
                    <option value="11-18">11-18</option>
                    <option value="19+">19+</option>
                    <option value="lady-low">Lady Low</option>
                    <option value="lady-mid">Lady Mid</option>
                    <option value="lady-high">Lady High</option>
                    <option value="lady-beginner">Lady Beginner</option>
                    <option value="junior">Junior</option>
                  </select>
                </div>

                <div className="mb-8">
                  <label className="form-label">What would you like to focus on?</label>
                  <textarea
                    name="playingLessonFocus"
                    className="form-textarea"
                    placeholder="Course management, mental game, shot selection..."
                  />
                </div>

                <GoldButton type="submit" variant="primary" size="lg" className="w-full">
                  Request Playing Lesson
                </GoldButton>

                <p className="text-center text-sm text-gray-600 mt-4 font-inter">
                  I&apos;ll contact you to confirm your playing lesson date and time.
                </p>
              </form>
            </CrystalCard>
          </ScrollReveal>
        </div>
      </section>

      {/* General Contact Form */}
      <section id="consultation" className="section-padding bg-surface">
        <div className="section-container max-w-4xl mx-auto">
          <div className="section-header mb-12 text-center">
            <h2 className="section-title font-playfair">Free Consultation</h2>
            <p className="section-subtitle font-inter">
              Want to learn more before booking? Schedule a free 15-minute call.
            </p>
          </div>

          <ScrollReveal>
            <CrystalCard className="p-8" hover="glow">
              <form>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="form-label">First Name</label>
                    <input type="text" name="firstName" className="form-input" placeholder="James" />
                  </div>
                  <div>
                    <label className="form-label">Last Name</label>
                    <input type="text" name="lastName" className="form-input" placeholder="Whittaker" />
                  </div>
                </div>

                <div className="mb-6">
                  <label className="form-label">Email Address</label>
                  <input type="email" name="email" className="form-input" placeholder="james@example.com" />
                </div>

                <div className="mb-6">
                  <label className="form-label">Phone Number</label>
                  <input type="tel" name="phone" className="form-input" placeholder="07810 647976" />
                </div>

                <div className="mb-6">
                  <label className="form-label">Topic</label>
                  <select className="form-input">
                    <option value="">What would you like to discuss?</option>
                    <option value="general">General questions</option>
                    <option value="pricing">Pricing and packages</option>
                    <option value="scheduling">Scheduling questions</option>
                    <option value="other">Other</option>
                  </select>
                </div>

                <div className="mb-8">
                  <label className="form-label">Question or Message</label>
                  <textarea
                    name="message"
                    className="form-textarea"
                    placeholder="How can I help?"
                  />
                </div>

                <GoldButton type="submit" variant="secondary" size="lg" className="w-full">
                  Request Call
                </GoldButton>

                <p className="text-center text-sm text-gray-600 mt-4 font-inter">
                  I&apos;ll get back to you within 24 hours to schedule your free consultation.
                </p>
              </form>
            </CrystalCard>
          </ScrollReveal>
        </div>
      </section>

      {/* FAQ */}
      <section className="section-padding bg-black">
        <div className="section-container max-w-3xl mx-auto">
          <div className="section-header mb-12">
            <h2 className="section-title font-playfair">Contact FAQs</h2>
            <p className="section-subtitle font-inter">Quick answers to common questions</p>
          </div>

          <div className="space-y-4">
            {[
              { q: "Do you offer trial lessons?", a: "Yes! I offer single sessions for anyone who wants to try my coaching before committing to a package. Many golfers use this as a swing assessment and come back for more." },
              { q: "What's the best way to contact you?", a: "You can email me at james@jwgolf.co.uk or call/text at 07810 647976. I'm usually available during weekday evenings and weekends for coaching sessions." },
              { q: "Is online coaching as effective as in-person?", a: "Absolutely. My online coaching uses the same One Clear Fix Method as in-person sessions. The key difference is delivery — online coaching gives you video analysis and personalized feedback without requiring you to travel." },
              { q: "What happens after I book?", a: "After booking, I'll send you a confirmation email with details about your session. For in-person lessons, I'll provide facility directions. For online coaching, I'll share upload instructions. For fittings, I'll ask about your current equipment." },
              { q: "Where are you located?", a: "I'm based in Leeds, Yorkshire. My private coaching facility has both indoor and outdoor hitting areas, Trackman, high-speed cameras, and all the technology for comprehensive swing analysis." },
              { q: "Can you travel for lessons?", a: "Yes! I offer travel services for groups, clubs, and special events. Please contact me for availability and pricing for travel lessons." },
            ].map((faq, i) => (
              <ScrollReveal key={i} delay={i * 80}>
                <details className="bg-surface-card rounded-xl overflow-hidden border border-white/[0.08] group">
                  <summary className="p-4 cursor-pointer flex justify-between items-center font-medium text-white hover:bg-white/[0.03] transition-colors font-playfair">
                    <span>{faq.q}</span>
                    <span className="group-open:rotate-180 transition-transform">▼</span>
                  </summary>
                  <div className="p-4 pt-0 text-gray-500 font-inter">{faq.a}</div>
                </details>
              </ScrollReveal>
            ))}
          </div>
        </div>
      </section>

      {/* Map / Location */}
      <section className="section-padding bg-black">
        <div className="section-container">
          <ScrollReveal>
            <CrystalCard className="overflow-hidden" hover="glow">
              <div className="aspect-video bg-surface-card flex items-center justify-center">
                <div className="text-center">
                  <div className="text-6xl mb-4">📍</div>
                  <h3 className="text-2xl font-playfair font-bold text-white mb-2">Leeds, Yorkshire</h3>
                  <p className="text-gray-400 font-inter">Private coaching facility</p>
                  <p className="text-sm text-gray-600 mt-4 font-inter">
                    Exact address provided upon booking confirmation
                  </p>
                </div>
              </div>
              <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <h4 className="font-bold text-white mb-2 font-playfair">Parking</h4>
                  <p className="text-sm text-gray-500 font-inter">
                    Private parking available at facility
                  </p>
                </div>
                <div className="text-center">
                  <h4 className="font-bold text-white mb-2 font-playfair">Public Transport</h4>
                  <p className="text-sm text-gray-500 font-inter">
                    10 minutes from Leeds city center
                  </p>
                </div>
                <div className="text-center">
                  <h4 className="font-bold text-white mb-2 font-playfair">Accessibility</h4>
                  <p className="text-sm text-gray-500 font-inter">
                    Fully accessible facility
                  </p>
                </div>
              </div>
            </CrystalCard>
          </ScrollReveal>
        </div>
      </section>

      {/* Emergency Contact */}
      <section className="section-padding bg-accent relative overflow-hidden">
        <div
          className="absolute inset-0 opacity-15"
          style={{
            background: `radial-gradient(ellipse 60% 50% at 50% 50%, rgba(255,255,255,0.1) 0%, transparent 60%)`,
          }}
        />
        <div className="section-container relative z-10 text-center">
          <ScrollReveal>
            <h2 className="text-3xl md:text-4xl font-playfair font-bold mb-6 text-white">
              Have A Question?
            </h2>
          </ScrollReveal>
          <ScrollReveal delay={100}>
            <p className="text-xl text-white/80 max-w-2xl mx-auto mb-8 font-inter">
              If you have a question that&apos;s not covered above, please don&apos;t hesitate
              to contact me directly.
            </p>
          </ScrollReveal>
          <ScrollReveal delay={200}>
            <a href="mailto:booking@jameswhittakergolf.com" className="inline-block px-8 py-4 bg-black text-accent font-bold rounded-sm hover:bg-black/80 transition-colors text-xl font-playfair">
              booking@jameswhittakergolf.com
            </a>
          </ScrollReveal>
          <ScrollReveal delay={300}>
            <p className="text-sm text-white/70 mt-4 font-inter">
              Or call/text: 07810 647976
            </p>
          </ScrollReveal>
        </div>
      </section>

      {/* CTA */}
      <section className="section-padding relative overflow-hidden">
        <AuroraBackground className="absolute inset-0" />
        <div className="section-container relative z-10 text-center">
          <ScrollReveal>
            <h2 className="text-4xl md:text-6xl font-playfair font-bold mb-6 text-white">
              Let&apos;s Fix Your Game
            </h2>
          </ScrollReveal>
          <ScrollReveal delay={100}>
            <p className="text-xl text-gray-400 max-w-2xl mx-auto mb-10 font-inter">
              Book your session today and start your journey to better golf with
              the One Clear Fix Method.
            </p>
          </ScrollReveal>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="https://calendly.com/james-whittaker-golf">
              <GoldButton variant="primary" size="lg" pulse>
                Book Directly
              </GoldButton>
            </Link>
            <Link href="#booking">
              <GoldButton variant="glass" size="lg">
                Fill Out Form
              </GoldButton>
            </Link>
          </div>
        </div>
      </section>
    </div>
  );
}